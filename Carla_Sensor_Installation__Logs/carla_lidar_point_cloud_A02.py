...
def process_lidar(point_cloud):
    ...
    # Splat each point as a small block so the cloud reads as dense
    # rather than single, hard-to-see pixels (nearer points get a
    # slightly larger splat).
    near_mask = cam_z_vis[order] < 25.0

    for dx, dy in ((0, 0), (1, 0), (0, 1), (1, 1)):
        xs = np.clip(px + dx, 0, IMAGE_WIDTH - 1)
        ys = np.clip(py + dy, 0, IMAGE_HEIGHT - 1)
        image[ys, xs] = colors

    # Extra splat radius for near points only.
    for dx, dy in ((-1, 0), (0, -1), (-1, -1), (2, 0), (0, 2)):
        xs = np.clip(px[near_mask] + dx, 0, IMAGE_WIDTH - 1)
        ys = np.clip(py[near_mask] + dy, 0, IMAGE_HEIGHT - 1)
        image[ys, xs] = colors[near_mask]
